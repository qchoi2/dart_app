...
2. `MCP 설치.bat`을 더블클릭합니다.
3. `.env`가 없으면 열린 메모장에 OpenDART API 키를 입력하고 닫습니다.
4. 설치가 끝나면 Claude Desktop을 완전히 종료했다가 다시 실행합니다.

직접 공시검색만 실행하려면 `리서치.bat`을 사용하거나 아래 명령을 입력합니다.

```powershell
python dart_research.py --keyword 상계납입 --bgn 20260101 --end 20260714 --report 유상증자결정 --max-docs 200 --out output/results
```

회사코드 없는 공시검색은 한 번에 최대 3개월까지 가능하지만, 이 프로젝트는 긴 기간을 달력 기준 3개월씩 자동 분할해 검색한 뒤 결과를 최신순으로 합칩니다.
...
| `MCP 설치.bat` | 패키지 설치·API 키 확인·Claude MCP 등록을 한 번에 수행 |
| `리서치.bat` | 간단 실행기 시작 |
