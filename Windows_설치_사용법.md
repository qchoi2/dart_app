...
파일 내용을 아래로 만들고, **경로**와 **키** 두 곳만 본인 값으로 바꾸세요.
...
| `DART_API_KEY 없음` 오류 | 설정의 `env.DART_API_KEY` 값 확인 |
| 결과가 느림/많음 | 자연어로 "max_docs 200으로 제한해서" 라고 요청 |
| 본문이 비어있는 공시 | 열람제한 문서일 수 있음(자동 건너뜀) |

## ⚠️ 보안
이 대화에 노출된 키는 **재발급**해서 쓰세요. 키는 설정의 `env` 에만 넣고 코드/문서에 남기지 마세요.
